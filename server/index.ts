...
app.post("/api/datasources/test", requireAuth, async (_req, res) => {
  const start = Date.now();
  try {
    await getMeta();
    res.json({
    ...
    const message = err instanceof Error ? err.message : String(err);
    logger.error("datasource_test_error", { error: message });
    res.json({
      success: false,
...
// --- Deploy files endpoint (for bon pull) ---

app.get("/api/deploy/files", requireAuth, (_req, res) => {
  const files: Record<string, string> = {};

  for (const sub of ["cubes", "views"]) {
    const dir = path.join(config.modelDir, sub);
    if (!fs.existsSync(dir)) continue;

    for (const file of fs.readdirSync(dir)) {
      if (file.endsWith(".yaml") || file.endsWith(".yml")) {
        const content = fs.readFileSync(path.join(dir, file), "utf-8");
        files[`bonnard/${sub}/${file}`] = content;
      }
    }
  }
...
// Return 404 for .well-known requests — prevents MCP clients (e.g. Claude Desktop)
// from mistaking the SPA catch-all for an OAuth2 authorization server.
